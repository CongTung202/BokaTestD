...
class ctKhachHangController extends Controller
{
    ...
    public function ctEdit($id) {
        $ctKhachHang = ctKhachHangModel::find($id);
        return view('ctAdmin.ctKhachHang.ct-edit', ['ctKhachHang' => $ctKhachHang]);
    }
    # edit submit
    public function ctEditSubmit(Request $request, $id)
{
    ...
    $request->validate([
        'ctMaKhach' => 'required|string|max:255',
        'ctTenKhach' => 'required|string|max:255',
        'ctEmail' => 'required|email',
        'ctMatKhau' => 'required|string|max:255',
        'ctDienThoai' => 'required|string|max:15',
        'ctDiaChi' => 'required|string|max:255',
    ...
    $ctKhachHang = ctKhachHangModel::find($id);
    if (!$ctKhachHang) {
        return redirect()->back()->withErrors(['msg' => 'Khách hàng không tồn tại.']);
    }
    // Update the model's attributes
    $ctKhachHang->ctMaKhachHang = $request->input('ctMaKhach');
    $ctKhachHang->ctHoTenKhachHang = $request->input('ctTenKhach');
    $ctKhachHang->ctEmail = $request->input('ctEmail');
    $ctKhachHang->ctMatKhau = $request->input('ctMatKhau');
    $ctKhachHang->ctDienThoai = $request->input('ctDienThoai');
    $ctKhachHang->ctDiaChi = $request->input('ctDiaChi');
    $ctKhachHang->ctTrangThai = $request->input('ctTrangThai');
    // Save the changes
    $ctKhachHang->save(); 
    // Redirect to the list page with a success message
    return redirect('/ctAdmin/Khach/ct-list')->with('success', 'Cập nhật khách hàng thành công.');
}
